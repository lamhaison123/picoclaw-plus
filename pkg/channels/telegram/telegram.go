...
func (c *TelegramChannel) handleCollaborativeCommand(ctx context.Context, chatID int64, content string) bool {
	...
	// Use collaborative.HandleCommand
	response, handled := collaborative.HandleCommand(c.chatManager, chatID, c, content, nil)
	if handled {
		if _, err := c.bot.SendMessage(c.ctx, tu.Message(tu.ID(chatID), response)); err != nil {
			logger.ErrorCF("telegram", "Failed to send command response", map[string]any{
				"command": content,
				"chat_id": chatID,
				"error":   err.Error(),
			})
