...
func (t *ExecTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	// Check if this is a grep command with exit code 1 (no matches found)
	isGrepNoMatch := false
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			exitCode := exitErr.ExitCode()
			// Check if command contains grep and exit code is 1
			if exitCode == 1 && strings.Contains(command, "grep") {
				isGrepNoMatch = true
				// For grep exit code 1, treat as success with no matches
				if output == "" {
