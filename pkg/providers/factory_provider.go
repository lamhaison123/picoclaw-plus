...
func getBreaker(providerName string) CircuitBreaker {
	breakersMu.RLock()
	cb, ok := breakers[providerName]
	breakersMu.RUnlock()
	if ok {
		return cb
	}
	...
	// Double check
	if cb, ok = breakers[providerName]; ok {
		return cb
	}

	cb = NewCircuitBreaker(DefaultBreakerConfig())
	breakers[providerName] = cb
	return cb
}

// WrapWithCircuitBreaker wraps a provider with a circuit breaker.
func WrapWithCircuitBreaker(provider LLMProvider, name string) LLMProvider {
	cb := getBreaker(name)
	return NewCircuitBreakerProvider(provider, cb, name)
}
