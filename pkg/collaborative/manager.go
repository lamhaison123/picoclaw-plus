...
//
// Deprecated: Use ManagerV2 instead. This implementation lacks:
//   - Cascade mention fix (infinite loop prevention)
//   - Depth limit enforcement
//   - Cycle detection
//   - Idempotency (duplicate execution prevention)
//   - Dispatch tracking
//
// ManagerV2 provides all these features plus better error handling and logging.
// This type will be removed in v2.0.0.
//
// Migration:
//
//	old: manager := collaborative.NewManager()
//	new: manager := collaborative.NewManagerV2()
type Manager struct {
...
//
// Deprecated: Use NewManagerV2() instead for better features including:
//   - Cascade mention fix
//   - Depth limit enforcement
//   - Cycle detection
//   - Idempotency
...
You can mention other team members using @role format (e.g., @architect, @developer).`,
		...
		go func(r string) {
			// Use background context with timeout for agent execution
			execCtx, execCancel := context.WithTimeout(context.Background(), 60*time.Second)
			defer execCancel()

			result, err := teamManager.ExecuteTaskWithRole(execCtx, teamID, prompt, r)
			...
			if err != nil {
				...
				defer sendCancel()
				platform.SendMessage(sendCtx, fmt.Sprintf("%d", chatID), errorMsg)
				return
			}
			...
			// Send to platform with timeout
			sendCtx, sendCancel := context.WithTimeout(context.Background(), 10*time.Second)
			defer sendCancel()
			sendErr := platform.SendMessage(sendCtx, fmt.Sprintf("%d", chatID), formattedMsg)
			...
			if len(mentionsInResponse) > 0 {
				...
				if len(newMentions) > 0 {
					...
					for _, mentionedRole := range newMentions {
						...
						// Update context with latest messages
						currentContext := session.GetContextAsString()
						prompt := fmt.Sprintf(`%s
...
You are @%s. You can mention other team members using @role format.`,
							currentContext, teamRoster, r, mentionedRole)

						// Execute in new goroutine
						go func(targetRole string) {
							// Use background context for execution
							execCtx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
							defer cancel()

							result, err := teamManager.ExecuteTaskWithRole(execCtx, teamID, prompt, targetRole)
							if err != nil {
							...
							sendCtx, sendCancel := context.WithTimeout(context.Background(), 10*time.Second)
							defer sendCancel()
							platform.SendMessage(sendCtx, fmt.Sprintf("%d", chatID), formattedMsg)
