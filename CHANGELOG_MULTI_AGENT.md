...
- **Fixed**: Agent treating grep exit code 1 as error
- **Issue**: `grep` returns exit code 1 when no matches found (valid result, not error)
- **Solution**: Special handling for grep commands - exit code 1 treated as success with "(no matches found)" message
- **Impact**: Cleaner logs, better agent decision making, improved UX
- **Files**: `pkg/tools/shell.go`, `pkg/tools/shell_grep_test.go`

#### Email Detection in Mentions
- **Fixed**: Email addresses being detected as @mentions
- **Issue**: `spawnacc2@gmail.com` extracted `@gmail` as mention
- **Solution**: Updated regex to `(?:^|[^\w.])@(\w+)` + email domain filtering
...
- **Fixed**: Directory sync errors silently ignored
- **Issue**: `dirFile.Sync()` error was ignored with `_`
- **Solution**: Added warning log for sync errors
- **Impact**: Better observability of disk issues
- **Files**: `pkg/tools/filesystem.go`

#### Nil Tools Registry in Subagent
- **Fixed**: Potential nil pointer when tools registry is nil
- **Issue**: `sm.tools` could be nil when passed to RunToolLoop
- **Solution**: Initialize empty ToolRegistry if nil
...
- **Files**: `pkg/collaborative/formatting.go`

#### Enhanced /who Command
- **Added**: Show all registered agents from team config
- **Added**: Agent descriptions and emojis
- **Added**: Team name display
- **Impact**: Better visibility of available agents
- **Files**: `pkg/channels/telegram/telegram.go`

### 🔧 Code Quality

#### Resource Management
- **Added**: Coordinator cleanup in team manager
- **Issue**: Potential goroutine leaks
- **Solution**: `defer coordinator.Shutdown()` in ExecuteTask methods
- **Files**: `pkg/team/manager.go`

### 📝 Documentation
- **Added**: `GREP_EXIT_CODE_FIX.md` - Detailed grep fix documentation
- **Updated**: `IMPROVEMENTS_SUMMARY.md` - Complete improvements summary
- **Updated**: `README.md` - Added /who command documentation

### 🧪 Testing
- **Added**: Comprehensive grep exit code tests (12 test cases)
- **Added**: Email detection tests (12 test cases)
- **Coverage**: 100% pass rate on all new tests

---

## [1.1.0] - 2026-03-05

### 🎉 New Feature: IRC-Style Collaborative Chat

Added native IRC-style collaborative multi-agent chat for Telegram, allowing multiple agents to participate in conversations within a single chat using @mentions.

### Added
- **Collaborative Chat Integration**: Native Go implementation integrated directly into `pkg/channels/telegram`
  - @mention-based routing (e.g., `@architect`, `@developer`)
  - Parallel agent execution for multiple mentions
  - Shared conversation context across all agents
  - IRC-style message formatting: `[session-id] emoji ROLE: message`
  - Session management per chat with configurable context window

- **TeamManager Integration**: 
  - Added `TeamManager` interface to `pkg/channels/manager.go`
  - TeamManager created in gateway and injected into ChannelManager
  - Accessible to all channels via interface pattern
  - Proper lifecycle management with team memory and agent executor

- **Configuration Support**:
  - Added `collaborative_chat` section to Telegram config
  - Fields: `enabled`, `default_team_id`, `max_context_length`, `role_map`
  - Updated `config/config.example.json` with example configuration

- **Documentation**:
  - Complete guide: `docs/COLLABORATIVE_CHAT.md`
  - Quick start: `docs/COLLABORATIVE_CHAT_QUICKSTART.md`
  - Example team config: `templates/teams/collaborative-dev-team.json`
  - API documentation: `pkg/collaborative/README.md`

- **Session Management**:
  - `CollaborativeChatManager` manages sessions per chat
  - `CollaborativeChatSession` tracks conversation history and agent states
  - Automatic session creation and context trimming
  - Session ID generation based on chat ID

- **Commands**:
  - `/who` - Show team status, all registered agents, and active agents
  - `/help` - Show available commands
  - Enhanced status display with emojis and descriptions

### Changed
- **TelegramChannel**: Modified `handleMessage()` to detect @mentions and route to collaborative handler
- **Gateway Initialization**: Added TeamManager creation with provider, executor, and memory setup

### Removed
- **Python IRC Gateway Skill**: Native implementation replaces the Python skill approach
- **collaborative_commands.go**: Removed in favor of simpler integrated approach

### Technical Details
- Architecture: User Message → Extract @mentions → Get/Create Session → Execute Agents (parallel) → Format & Send
- Message format: `[abc123] 🏗️ ARCHITECT: response text`
- Role emojis: 🏗️ architect, 💻 developer, 🧪 tester, 📋 manager, ⚙️ devops, 🎨 designer
- Parallel execution via goroutines for fast multi-agent responses
- Context passed to agents includes full conversation history
- Interface-based design avoids circular dependencies

### Benefits
- Single Telegram bot token (no need for multiple bots)
- Native Go performance and lower memory footprint
- Better integration with PicoClaw's team system
- Faster response times compared to Python skill
- Cleaner architecture with proper separation of concerns

### Usage Example
```
User: @architect @developer How should we implement authentication?
...
1. Removing Python skill from `~/.picoclaw/workspace/skills/irc-gateway/`
2. Configuring `collaborative_chat` in `config.json`
3. Creating team configuration file
...
### Added
- **Platform-Agnostic Collaborative Chat Package** (`pkg/collaborative/`)
  - Extracted collaborative chat logic into reusable package
  - Platform interface for multi-platform support (Telegram, IRC, Discord, etc.)
  - Session management with conversation context
  - @mention extraction and routing
  - IRC-style message formatting with role emojis
  - Team roster building
  - Agent-to-agent communication support
  - Comprehensive API documentation

### Changed
- **Telegram Channel Refactoring**
  - Simplified `pkg/channels/telegram/telegram.go` (~150 lines removed)
  - Now uses `pkg/collaborative/` package
  - Implements Platform interface
  - Better error handling with user-friendly messages

### Fixed
- **Response Format Bug**
  - Enhanced `extractResponseText()` to handle complex nested structures
  - Agents now return clean text instead of raw map structures
  - Better type checking and recursion
...
- **Code Organization**
  ...
  - Easier to test with mock interfaces
  - Prepared for IRC and other platform integrations

### Documentation
- Added `pkg/collaborative/README.md` - Complete API documentation
- Updated `docs/COLLABORATIVE_CHAT.md` - Added developer guide and package info
- Updated `.kiro/steering/structure.md` - Added collaborative package
- Created analysis documents for PR #1138 (IRC integration)
- Created refactoring plan and summary documents

### Technical Details
- **Files Created**: 6 new files in `pkg/collaborative/` (~660 lines)
- **Files Modified**: `pkg/channels/telegram/telegram.go` (simplified)
- **Build Status**: ✅ All builds successful
- **Backward Compatibility**: ✅ 100% compatible, no breaking changes
- **Performance**: No degradation, same logic reorganized

### Migration Notes
For developers integrating collaborative chat:
- Old: `extractMentions(text)` → New: `collaborative.ExtractMentions(text)`
- Old: `getRoleEmoji(role)` → New: `collaborative.GetRoleEmoji(role)`
- Old: Direct session access → New: Use Platform interface

### Future Work
- [ ] Merge PR #1138 (IRC channel integration)
- [ ] Implement IRC collaborative chat
- [ ] Add Discord collaborative chat support
- [ ] Cross-platform message routing
- [ ] Persistent session storage
- [ ] Unit tests for collaborative package
